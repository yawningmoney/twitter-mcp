...
class TwitterServer {
  ...
  private setupToolHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      await delay(ms(`${Math.random() * 5}s`));
      ...
      if (request.params.name === 'post_tweet') {
        ...
        try {
          ...
          const response = await userClient.v2.tweet(status);
          return {
            content: [
              {
                type: 'text',
      ...
      if (request.params.name === 'read_timeline') {
        const { user_id } = request.params.arguments as { user_id: string };
        try {
          const response = await this.twitterClient.v2.userTimeline(user_id);
          return {
            content: [
              {
                type: 'text',
      ...
      if (request.params.name === 'send_direct_message') {
        const { recipient_id, text } = request.params.arguments as {
          recipient_id: string;
          text: string;
        };
        try {
          const response = await userClient.v2.sendDmToParticipant(recipient_id, {
            text,
          });
          return {
            content: [
      ...
      if (request.params.name === 'follow_user') {
        const { user_id } = request.params.arguments as { user_id: string };
        try {
          const response = await userClient.v2.follow(process.env.TWITTER_USER_ID!, user_id);
          return {
            content: [
              {
                type: 'text',
      ...
      if (request.params.name === 'unfollow_user') {
        const { user_id } = request.params.arguments as { user_id: string };
        try {
          const response = await userClient.v2.unfollow(process.env.TWITTER_USER_ID!, user_id);
          return {
            content: [
              {
                type: 'text',
      ...
      if (request.params.name === 'scrape_tweet_information') {
        ...
        if (!this.firecrawl) {
          throw new McpError(ErrorCode.InvalidRequest, 'Firecrawl API key is not configured');
        }
        try {
      ...
      if (request.params.name === 'get_trending_topics') {
        const { woeid } = request.params.arguments as { woeid: string };
        try {
          const response = await this.twitterClient.v1.trendsByPlace(woeid);
          return {
            content: [
              {
                type: 'text',
      ...
      if (request.params.name === 'like_tweet') {
        ...
        try {
          const response = await userClient.v2.like(process.env.TWITTER_USER_ID!, tweet_id);
          return {
            content: [
              {
                type: 'text',
      ...
      if (request.params.name === 'retweet') {
        const { tweet_id } = request.params.arguments as { tweet_id: string };
        try {
          const response = await userClient.v2.retweet(process.env.TWITTER_USER_ID!, tweet_id);
          return {
            content: [
              {
                type: 'text',
      ...
      if (request.params.name === 'comment_on_tweet') {
        ...
        try {
          const response = await userClient.v2.reply(text, tweet_id);
          return {
            content: [
              {
                type: 'text',
      ...
      if (request.params.name === 'get_followers') {
        const { user_id } = request.params.arguments as { user_id: string };
        try {
          const response = await this.twitterClient.v2.followers(user_id);
          return {
            content: [
              {
                type: 'text',
      ...
      if (request.params.name === 'get_following') {
        const { user_id } = request.params.arguments as { user_id: string };
        try {
          const response = await this.twitterClient.v2.following(user_id);
          return {
            content: [
              {
                type: 'text',
      ...
      if (request.params.name === 'get_klout_score') {
        const { username } = request.params.arguments as { username: string };
        if (!KLOUT_API_KEY) {
          throw new McpError(ErrorCode.InvalidRequest, 'Klout API key is not configured');
        }
        try {
          const response = await axios.get(`http://api.klout.com/v2/identity.json/twitter?screenName=${username}&key=${KLOUT_API_KEY}`);
          const kloutId = response.data.id;
          const scoreResponse = await axios.get(`http://api.klout.com/v2/user.json/${kloutId}/score`);
          return {
            content: [
              {
                type: 'text',
      ...
      if (request.params.name === 'set_twitter_tokens') {
        const { access_token, access_token_secret } = request.params.arguments as {
          access_token: string;
